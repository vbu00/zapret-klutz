...
pub fn scan_game_from_log(
    ...
    let note = match &pick.process {
        Some(game) => {
            ...
            // Фильтр трогаем, только если в списке есть сети. Все пойманные
            // серверы могли оказаться облачными и не пойти в список — тогда
            // включать игровые порты незачем: обходу там не с чем работать, а
            // режим, который человек выбрал сам, мы бы молча поменяли.
            ...
            let mut n = if есть_сети {
                ...
                format!(
                    "Игра: {game}. Все её серверы, до которых дошёл сбор, стоят в облаке, а \
                     облачные сети делят с игрой пол-интернета, — в список они не пошли, и \
                     Game Filter не тронут."
                )
            };
            if режим != было {
                n.push_str(&format!(
