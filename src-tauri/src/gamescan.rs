...
pub enum Operator {
    ...
    /// Оператор известен, но за его адресами стоит пол-интернета. Ни его
    /// сети, ни /24 вокруг адреса не берём — только сам пойманный сервер.
...
/// Сервер из облака — одним адресом, без соседей.
///
/// Сеть облачного оператора — это пол-интернета, а /24 вокруг адреса — чужие
/// сайты в той же стойке. Но сам сервер, к которому подключалась игра, — это
/// именно её адрес, и взять его безопасно.
pub fn один_сервер(ip: &str) -> String {
    match ip.parse::<IpAddr>() {
...
///
/// Возвращает облачные серверы, взятые поштучно, готовыми к показу строками:
/// человеку надо знать, что соседний сервер того же облака в список не попал
/// и следующий матч может понадобиться поймать заново.
pub fn save_ips_to(
    root: &std::path::Path,
    target: Target,
    addrs: &[String],
    ...
    for a in addrs {
        ...
        match operator_of(a) {
            ...
            // Облако целиком не берём: его сети и /24 вокруг адреса — чужой
            // трафик, который обход начнёт ломать. Но сам пойманный сервер —
            // адрес игры, и он идёт в список один. Раньше не шёл и он, и игры
            // на облаках — Rocket League на Amazon — не собирались вовсе.
            Operator::Cloud { asn, .. } => {
                let name = holder_of(&asn);
                // Если адрес был пропущен прежней версией, больше он не пропущен.
                skipped.retain(|s| s.addr != *a);
                поштучно.push(format!(
                    "{a} ({})",
                    if name.is_empty() { format!("AS{asn}") } else { name.clone() }
                ));
                добавить_группу(&mut groups, asn, name, now, vec![один_сервер(a)]);
            }
            Operator::Unknown => {
...
mod unit_tests {
    ...
    #[test]
    fn облачный_сервер_берётся_один() {
        assert_eq!(один_сервер("35.181.49.176"), "35.181.49.176/32");
        assert_eq!(один_сервер("2600:1f18::1"), "2600:1f18::1/128");
        // Второй сервер того же облака пополняет его группу, а не заводит
        // новую, и повтор не дублируется: сборов бывает несколько подряд.
        let amazon = || ("16509".to_string(), "Amazon.com, Inc.".to_string());
        let mut groups = Vec::new();
        let (asn, name) = amazon();
        добавить_группу(&mut groups, asn, name, 1, vec![один_сервер("35.181.49.176")]);
        let (asn, name) = amazon();
        добавить_группу(
            &mut groups,
            asn,
            name,
            2,
            vec![один_сервер("15.236.146.62"), один_сервер("35.181.49.176")],
        );
        assert_eq!(groups.len(), 1);
        assert_eq!(groups[0].nets, vec!["15.236.146.62/32", "35.181.49.176/32"]);
        // И запись в файл это переживает.
        let текст = merge_groups("", &groups, &[]);
        assert_eq!(parse_groups(&текст).0, groups, "{текст:?}");
        assert!(!текст.contains(EMPTY_STUB), "{текст:?}");
    }

    #[test]
    fn облако_опознаётся_по_числу_сетей() {
